...
class Demo_Tester(object):
    def __init__(self, cfg: EasyConfig) -> None:
        ...
        self.save_root = osp.join(cfg.save_root, "test_results", f"epoch_{self.epoch}_demo")

        if osp.exists(self.save_root):
            logging.warning(f"Test result dir {self.save_root} already exists, deleting...")
    ...
    def _record_cfg(self, cfg: EasyConfig) -> None:
        ...
        with open(osp.join(cfg_dir, "demo_config.yaml"), "w") as wf:
    ...
    def _build_dataloaders(self, data_cfg: EasyConfig) -> Tuple[DataLoader, DataLoader]:
        # [] check the data loader and check data. 
...
def load_cfg():
    def combine_cfg(train_cfg_path, test_cfg_path):
        """
        ...
        # test cfg
        new_cfg = EasyConfig()
        new_cfg.load(test_cfg_path)
        test_cfg.update(new_cfg.dict())

        return test_cfg
    
    args = parse_args()
    idgc_test_cfg = combine_cfg(
    ...
    return idgc_test_cfg, qgc_test_cfg
