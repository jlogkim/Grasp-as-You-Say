...
class Object_mesh:
    ...
    def get_obj_mesh(self, oid, use_downsample=False, key="align", mesh_path = None):
...
if __name__ == "__main__":
    ...
    for i in tqdm.tqdm(range(0, len(data_list), 1)):
        ...
        guidance_details = data['guidance'].split(",")[0].replace(" ","_")

        obj_name, object_mesh = object_class.get_obj_mesh(data["obj_id"], mesh_path=data['mesh_path'] if 'mesh_path' in data else None)
        ...
        for j in range(predictions.shape[0]):
            ...
            break
        save_mesh_path = os.path.join(save_root, f"{only_code}_{str(vis_dict[only_code])}_{guidance_details}.obj")
        print(f"Export mesh to {save_mesh_path}")
        combined_mesh.export(os.path.join(save_root, f"{only_code}_{str(vis_dict[only_code])}_{guidance_details}.obj"))
